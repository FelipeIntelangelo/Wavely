...
## 7. Soft Delete y Anonimizaci�n de Usuarios

Para evitar cuellos de botella de integridad referencial (ej: borrar f�sicamente un usuario que posee un podcast inactivo), se implement� un modelo de Soft Delete H�brido en `UserService`.

- **Hard Delete Selectivo**: Al eliminar la cuenta, se borran f�sicamente asociaciones menores (Playlists, Comentarios, Historial, Valoraciones, Followers).
- **Anonimizaci�n**: La fila de `User` se mantiene pero sus datos PII se sobreescriben (nombre a 'Usuario Eliminado', email/username a 'deleted_id'). La contrase�a se invalida.
- **Podcasts Activos**: El borrado se bloquea si el usuario posee al menos un podcast **activo**. Si tiene podcasts inactivos, el proceso contin�a y la cuenta anonimizada act�a como due�a fantasma de los mismos.
